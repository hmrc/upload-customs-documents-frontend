...
class JourneyLockingSpec extends UnitSpec with MockFactory with LogCapturing {
  ...
  class fixture {
    ...
    object TestJourneyLocking extends JourneyLocking {
      ...
      override val lockReleaseCheckInterval: Duration     = Duration(100, TimeUnit.MILLISECONDS)
      override val lockTimeout: Duration                  = Duration(1000, TimeUnit.MILLISECONDS)
    }
    ...
    def mockTakeLock(
      ...
      (mockLockRepo
        ...
        .expects(journeyId.value, *, Duration(1000, TimeUnit.MILLISECONDS))
        .returns(isTaken)
