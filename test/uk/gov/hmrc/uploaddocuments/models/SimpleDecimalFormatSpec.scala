...
class SimpleDecimalFormatSpec extends AnyWordSpec with Matchers {
  ...
  "SimpleDecimalFormat" should {
    "serialize an entity as a number" in {
      ...
      format.writes(A(101)) shouldBe JsNumber(101)
    }

    "de-serialize a number as an entity" in {
      format.reads(JsNumber(2)) shouldBe JsSuccess(A(2))
      format.reads(JsNumber(0)) shouldBe JsSuccess(A(0))
      format.reads(JsNumber(101)) shouldBe JsSuccess(A(101))
      format.reads(JsNull) shouldBe a[JsError]
      format.reads(Json.obj()) shouldBe a[JsError]
      format.reads(JsString("2")) shouldBe a[JsError]
